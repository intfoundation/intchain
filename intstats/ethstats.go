...
// Package intstats implements the network stats reporting service.
package intstats
...
type Service struct {
	...
	eth    *intprotocol.Ethereum // Full Ethereum service if monitoring a full node
...
func (s *Service) login(conn *websocket.Conn) error {
	...
	if info := infos.Protocols[p.Name]; info != nil {
		network = fmt.Sprintf("%d", info.(*intprotocol.NodeInfo).Network)
...
func (s *Service) reportLatency(conn *websocket.Conn) error {
	// Send the current time to the intstats server
	start := time.Now()
	...
	log.Trace("Sending measured latency to intstats", "latency", latency)
...
func (s *Service) reportBlock(conn *websocket.Conn, block *types.Block) error {
	...
	log.Trace("Sending new block to intstats", "number", details.Number, "hash", details.Hash)
...
func (s *Service) reportHistory(conn *websocket.Conn, list []uint64) error {
	...
	if len(history) > 0 {
		log.Trace("Sending historical blocks to intstats", "first", history[0].Number, "last", history[len(history)-1].Number)
	} else {
...
func (s *Service) reportPending(conn *websocket.Conn) error {
	...
	log.Trace("Sending pending transactions to intstats", "count", pending)
...
func (s *Service) reportStats(conn *websocket.Conn) error {
	...
	log.Trace("Sending node details to intstats")
