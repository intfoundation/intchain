...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"intstats", "explorer", "wallet", "faucet"} {
	...
	// If we have intstats running, ask whether to make the secret public or not
	if w.conf.ethstats != "" {
		fmt.Println()
		fmt.Println("Include intstats secret on dashboard (y/n)? (default = yes)")
