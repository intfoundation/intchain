...
ADD intstats.json /intstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../eth-net-intelligence-api && pm2 start /intstats.json)' >  explorer.sh && \
...
// explorerEthstats is the configuration file for the intstats javascript client.
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	files[filepath.Join(workdir, "intstats.json")] = ethstats.Bytes()
