...
// ethstatsDockerfile is the Dockerfile required to build an intstats backend
// and associated monitoring site.
var ethstatsDockerfile = `
FROM puppeth/intstats:latest

RUN echo 'module.exports = {trusted: [{{.Trusted}}], banned: [{{.Banned}}], reserved: ["yournode"]};' > lib/utils/config.js
`

// ethstatsComposefile is the docker-compose.yml file required to deploy and
// maintain an intstats monitoring site.
var ethstatsComposefile = `
version: '2'
services:
  intstats:
    build: .
    image: {{.Network}}/intstats{{if not .VHost}}
    ports:
      - "{{.Port}}:3000"{{end}}
    environment:
...
// deployEthstats deploys a new intstats container to a remote machine via SSH,
...
func deployEthstats(client *sshClient, network string, port int, secret string, vhost string, trusted []string, banned []string, nocache bool) ([]byte, error) {
	...
	// Build and deploy the intstats service
	if nocache {
...
// ethstatsInfos is returned from an intstats status check to allow reporting
...
// checkEthstats does a health-check against an intstats server to verify whether
// it's running, and if yes, gathering a collection of useful infos about it.
func checkEthstats(client *sshClient, network string) (*ethstatsInfos, error) {
	// Inspect a possible intstats container on the host
