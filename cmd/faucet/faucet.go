...
import (
	...
	"github.com/intfoundation/intchain/intstats"
	"github.com/intfoundation/intchain/les"
	...
	"github.com/intfoundation/intchain/p2p/discv5"
...
var (
	...
	statsFlag   = flag.String("intstats", "", "Ethstats network monitoring auth string")
...
type faucet struct {
	...
	client *intclient.Client   // Client connection to the Ethereum chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	// Assemble the intstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.LightEthereum
			ctx.Service(&serv)
			return intstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := intclient.NewClient(api)
...
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	for {
		...
		default:
			err = errors.New("Something funky happened, please open an issue at https://github.com/intfoundation/intchain/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
...
func authGitHub(url string) (string, string, common.Address, error) {
	...
	for _, file := range gist.Files {
		...
		if len(content) == 2+common.INTAddressLength*2 {
