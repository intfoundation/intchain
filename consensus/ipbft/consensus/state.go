...
func (cs *ConsensusState) proposerByVRF(headerHash common.Hash, validators []*types.Validator) (proposer int) {
	...
	hash := new(big.Int)
	hash.SetBytes(hv[:])
	//fmt.Printf("ConsensusState proposerByVRF hash %v", hash)
...
func (cs *ConsensusState) createProposalBlock() (*types.TdmBlock, *types.PartSet) {
	...
	if cs.blockFromMiner != nil {
		...
		if cs.Height == cs.Epoch.EndBlock {
			...
			fmt.Printf("createProposalBlock current height %v\n, start height %v\n", cs.Epoch.StartBlock, cs.Height)
			...
			fmt.Printf("createProposalBlock should propose next epoch %v, height %v\n", shouldProposeEpoch, cs.Height)
...
func (cs *ConsensusState) defaultSetProposal(proposal *types.Proposal) error {
	...
	if proposal.Round == cs.Round {
		...
		fmt.Printf("default set proposal2, proposal %v\n", proposal.Signature)
		fmt.Printf("default set proposal2, cs.proposerByRound(proposal.Round).Proposer.PubKey %v\n", cs.proposerByRound(proposal.Round).Proposer.PubKey)
