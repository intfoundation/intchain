...
func (sb *backend) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if header.Time.Cmp(big.NewInt(now().Unix())) > 0 {
		...
		//in intchain, avoid the time difference to tolerate time gap between nodes
...
func (sb *backend) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction,
	...
	// update validator status incloud block time and forbidden
...
// Main Chain:
// The total reward consists of the static block reward of the Epoch and total tx gas fee.
// Child Chain:
...
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, ep *epoch.Epoch, totalGasFee *big.Int) {
	...
	if totalProxiedDeposit.Sign() == 0 {
		...
		if commission > 0 {
			// commissionReward = delegateReward * commission / 100
			...
			// Add the commission to self reward  selfReward = selfReward + commissionReward
			selfReward.Add(selfReward, commissionReward)

			// Sub the commission from delegate reward  delegateReward = delegateReward - commissionReward
	...
	//state.MarkAddressReward(header.Coinbase)
