...
type Sync struct {
	database intdb.Reader             // Persistent database to check for existing entries
...
func NewSync(root common.Hash, database intdb.Reader, callback LeafCallback) *Sync {
	ts := &Sync{
		database: database,
...
func (s *Sync) Commit(dbw intdb.Writer) (int, error) {
