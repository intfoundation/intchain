...
type ethApis struct {
	loaded  uint
	backend *intprotocol.IntChain
	//n *node.Node
	pubEthApi *intprotocol.PublicEthereumAPI
}
...
func ReloadEthApi(n *node.Node, backend *intprotocol.IntChain) {
	...
	pubEthApiType := reflect.TypeOf(&intprotocol.PublicEthereumAPI{})
	...
	for i := 0; i < len(apis); i++ {
		...
		if svrType.String() == str && svrType.PkgPath() == pkg {
			EthApi.pubEthApi = api.Service.(*intprotocol.PublicEthereumAPI)
		}
	}

	//PrintType(reflect.TypeOf(&intprotocol.PublicEthereumAPI{}))
}
...
func Coinbase() (string, error) {
	...
	return "", errors.New("PublicEthereumAPI not included")
}
...
func PrintType(t reflect.Type) {
	...
	//fmt.Println("Implements(*intprotocol.PublicEthereumAPI)    :", t.Implements(reflect.TypeOf(&intprotocol.PublicEthereumAPI{})))  // 是否实现了指定接口
	//fmt.Println("ConvertibleTo(*intprotocol.PublicEthereumAPI) :", t.ConvertibleTo(reflect.TypeOf(&intprotocol.PublicEthereumAPI{}))) // 是否可转换为指定类型
	//fmt.Println("AssignableTo(*intprotocol.PublicEthereumAPI)  :", t.AssignableTo(reflect.TypeOf(&intprotocol.PublicEthereumAPI{})))  // 是否可赋值给指定类型的变量
